...
void prefix_sum(const float* input, float* output, size_t n) {
    ...
    // Main loop: Process 16 elements at a time (2x AVX2 blocks)
    // Unrolling helps hide the latency of the prefix sum dependency chain
    for (; i + 15 < n; i += 16) {
        // Block 1
        ...
        // Extract last element of Block 1 to feed Block 2
