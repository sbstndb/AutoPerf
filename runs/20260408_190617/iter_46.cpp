...
int find(const int* data, int size, int value) {
    ...
    // This maximizes throughput on Intel Ultra 7 by saturating load ports and ILP.
    while (end - ptr >= 64) {
        ...
        if (!_mm256_testz_si256(combined, combined)) {
            ...
            auto get_idx = [&](__m256i c, int offset) {
                int mask = _mm256_movemask_ps(_mm256_castsi256_ps(c));
                return mask ? (int)(ptr - data) + offset + __builtin_ctz(mask) : -1;
            };
            ...
            if ((res = get_idx(c3, 24)) != -1) return res;
            if ((res = get_idx(c4, 32)) != -1) return res;
            if ((res = get_idx(c5, 40)) != -1) return res;
            if ((res = get_idx(c6, 48)) != -1) return res;
            return get_idx(c7, 56);
        }
        ptr += 64;
    }
