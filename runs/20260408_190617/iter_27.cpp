...
int find(const int* data, int size, int value) {
    ...
    if (size >= 8) {
        ...
        // Process 64 elements per iteration to maximize ILP and reduce branch frequency
        while (ptr <= end - 64) {
            ...
            __m256i m1 = _mm256_or_si256(or01, or23);
            __m256i m2 = _mm256_or_si256(or45, or67);
            __m256i combined = _mm256_or_si256(m1, m2);

            if (!_mm256_testz_si256(combined, combined)) {
                // Match found in this 64-element block
                auto get_idx = [&](__m256i cmp, const int* p) -> int {
                    int mask = _mm256_movemask_ps(_mm256_castsi256_ps(cmp));
                    return (mask != 0) ? (int)(p - data) + __builtin_ctz(mask) : -1;
                };
