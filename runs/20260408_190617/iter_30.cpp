...
int find(const int* data, int size, int value) {
    ...
    if (size >= 8) {
        ...
        for (; i <= size - 32; i += 32) {
            ...
            // Extract masks (1 bit per 32-bit lane)
            ...
            // Combine masks into a single 32-bit integer
            // Each m is 8 bits. Shift and OR to create a continuous bitmask.
            unsigned int combined_mask = (unsigned int)m0 | ((unsigned int)m1 << 8) | 
                                         ((unsigned int)m2 << 16) | ((unsigned int)m3 << 24);
