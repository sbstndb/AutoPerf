...
int find(const int* data, int size, int value) {
    ...
    // Process 32 elements per iteration (4 x 256-bit vectors)
    // This unroll factor balances ILP and code size for the Ultra 7 255U
    while (end - ptr >= 32) {
        ...
        // Combine masks using OR to check 32 elements at once
        ...
        if (!_mm256_testz_si256(combined, combined)) {
            // Match found in this 32-element block
            int mask0 = _mm256_movemask_ps(_mm256_castsi256_ps(c0));
            if (mask0 != 0) return (int)(ptr - data) + __builtin_ctz(mask0);
            
            int mask1 = _mm256_movemask_ps(_mm256_castsi256_ps(c1));
            if (mask1 != 0) return (int)(ptr - data) + 8 + __builtin_ctz(mask1);
            
            int mask2 = _mm256_movemask_ps(_mm256_castsi256_ps(c2));
            if (mask2 != 0) return (int)(ptr - data) + 16 + __builtin_ctz(mask2);
            
            int mask3 = _mm256_movemask_ps(_mm256_castsi256_ps(c3));
            return (int)(ptr - data) + 24 + __builtin_ctz(mask3);
        }
        ptr += 32;
    }
