...
int find(const int* data, int size, int value) {
    ...
    if (remaining >= 64) {
        ...
        while (remaining >= 64) {
            ...
            // Check first 32 elements
            int mask_low = _mm256_movemask_ps(_mm256_castsi256_ps(or_low));
            if (mask_low != 0) {
                int m;
                if ((m = _mm256_movemask_ps(_mm256_castsi256_ps(c0)))) return (int)(ptr - data) + __builtin_ctz(m);
                if ((m = _mm256_movemask_ps(_mm256_castsi256_ps(c1)))) return (int)(ptr - data) + 8 + __builtin_ctz(m);
                if ((m = _mm256_movemask_ps(_mm256_castsi256_ps(c2)))) return (int)(ptr - data) + 16 + __builtin_ctz(m);
                return (int)(ptr - data) + 24 + __builtin_ctz(_mm256_movemask_ps(_mm256_castsi256_ps(c3)));
            }

            // Check next 32 elements
            int mask_high = _mm256_movemask_ps(_mm256_castsi256_ps(or_high));
            if (mask_high != 0) {
                int m;
                if ((m = _mm256_movemask_ps(_mm256_castsi256_ps(c4)))) return (int)(ptr - data) + 32 + __builtin_ctz(m);
                if ((m = _mm256_movemask_ps(_mm256_castsi256_ps(c5)))) return (int)(ptr - data) + 40 + __builtin_ctz(m);
                if ((m = _mm256_movemask_ps(_mm256_castsi256_ps(c6)))) return (int)(ptr - data) + 48 + __builtin_ctz(m);
                return (int)(ptr - data) + 56 + __builtin_ctz(_mm256_movemask_ps(_mm256_castsi256_ps(c7)));
            }
