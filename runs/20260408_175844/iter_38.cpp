...
void prefix_sum(const float* input, float* output, size_t n) {
    ...
    // We interleave the intra-vector scans to hide instruction latency
    for (; i + 16 <= n; i += 16) {
        __m256 x1 = _mm256_loadu_ps(&input[i]);
        __m256 x2 = _mm256_loadu_ps(&input[i + 8]);

        // Step 1: Intra-lane scan for both vectors (Parallel)
        ...
        // Step 2: Cross-lane scan for both vectors
        ...
        // Step 4: Apply carry from x1 to x2
    ...
    // Scalar tail
    float carry_val = (i > 0) ? output[i - 1] : 0.0f;
    for (; i < n; ++i) {
        carry_val += input[i];
        output[i] = carry_val;
    }
