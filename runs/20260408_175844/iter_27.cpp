...
void prefix_sum(const float* input, float* output, size_t n) {
    ...
    for (; i + 15 < n; i += 16) {
        ...
        x1 = _mm256_add_ps(x1, _mm256_blend_ps(_mm256_setzero_ps(), 
                                _mm256_permute_ps(lane_sum1, 0xFF), 0xF0));
        
        // Apply global carry to x1 and store
        x1 = _mm256_add_ps(x1, v_carry);
        _mm256_storeu_ps(&output[i], x1);

        // Step 3: Cross-lane scan for x2
        __m256 lane_sum2 = _mm256_permute2f128_ps(x2, x2, 0x20);
        x2 = _mm256_add_ps(x2, _mm256_blend_ps(_mm256_setzero_ps(), 
                                _mm256_permute_ps(lane_sum2, 0xFF), 0xF0));

        // Step 4: Propagate carry from x1's last element to all of x2
