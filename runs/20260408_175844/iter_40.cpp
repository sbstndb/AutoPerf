...
void prefix_sum(const float* input, float* output, size_t n) {
    ...
    // Process 16 elements at a time with interleaved instructions for ILP
    for (; i + 16 <= n; i += 16) {
        __m256 x1 = _mm256_loadu_ps(&input[i]);
        __m256 x2 = _mm256_loadu_ps(&input[i + 8]);

        // Step 1: Intra-lane shifts (4 bytes)
        ...
        // Step 2: Intra-lane shifts (8 bytes)
        ...
        // Update global carry for next loop iteration
