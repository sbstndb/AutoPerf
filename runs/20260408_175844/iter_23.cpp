...
void prefix_sum(const float* input, float* output, size_t n) {
    ...
    for (; i + 16 <= n; i += 16) {
        ...
        lth_0 = _mm256_shuffle_ps(lth_0, lth_0, _MM_SHUFFLE(3, 3, 3, 3));
        x0 = _mm256_add_ps(x0, lth_0);

        // Parallel Prefix Sum for x1
        ...
        lth_1 = _mm256_shuffle_ps(lth_1, lth_1, _MM_SHUFFLE(3, 3, 3, 3));
        x1 = _mm256_add_ps(x1, lth_1);

        // Apply carry to x0
        x0 = _mm256_add_ps(x0, v_carry);

        // Calculate carry for x1 (last element of x0)
